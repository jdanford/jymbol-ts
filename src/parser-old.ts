...
export class Parser {
    ...
    constructor(symbolTable: SymbolTable, input: string) {
    ...
    static parse(symbolTable: SymbolTable, input: string): Expr {
        const parser = new Parser(symbolTable, input);
    ...
    atom(): Atom | undefined {
        ...
        return string ? this.symbolTable.intern(string) : undefined;
    }
    ...
    quoted(): Quoted<Expr> {
        ...
        if (quotedExpr === undefined) {
            this.fail(`Expected expression after \`${quoteChar}\``);
        }

        return new Quoted<Expr>(quoteType, quotedExpr);
    }
